...
int main()
{
	...
	// create sfml window
	sf::RenderWindow window(sf::VideoMode(startWidth, startHeight), "Common elements");
	...
	root->addChild(buttonList);
	{
		...
		auto h = root->addConstraint(Guider::ConstraintsContainer::Constraint::Orientation::Horizontal, buttonList, true);
		h->attachBetween(root, true, root, false, 0);
		h->setFlow(0.5);
		
		// setup vertical centering constraint
		auto v = root->addConstraint(Guider::ConstraintsContainer::Constraint::Orientation::Vertical, buttonList, true);
	...
	root->addChild(text);
	{
		...
		auto h = root->addConstraint(Guider::ConstraintsContainer::Constraint::Orientation::Horizontal, text, true);
		h->attachBetween(buttonList, false, root, false, true);
		h->setFlow(0.5);

		// stick to the top of button list
		auto v = root->addConstraint(Guider::ConstraintsContainer::Constraint::Orientation::Vertical, text, true);
